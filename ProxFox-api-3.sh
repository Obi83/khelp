...
fetch_proxies() {
    ...
    local original_ip=$(curl -s https://api.ipify.org)
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if [ -n "$response" ]; then
            ...
            if [ -n "$valid_proxies" ]; then
                echo "$valid_proxies" | while read -r proxy; do
                    local proxy_ip=$(curl -x "socks5://$proxy" -s https://api.ipify.org)
                    if [ "$original_ip" != "$proxy_ip" ]; then
                        if ! grep -q "^$proxy$" "$proxy_list_file"; then
                            echo "$proxy" >> "$proxy_list_file"
                        fi
                    else
                        log $LOG_LEVEL_ERROR "Proxy $proxy is not anonymous." "$UPDATE_LOG_FILE"
                    fi
                done
                log $LOG_LEVEL_INFO "Fetched $(wc -l < "$proxy_list_file") valid and anonymous proxies." "$UPDATE_LOG_FILE"
                return 0
            else
