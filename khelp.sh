...
# Update the system
update_system() {
    log $LOG_LEVEL_INFO "Updating and upgrading system" "/var/log/khelp_proxy.log"
    local attempts=0
    local max_attempts=3

    while [ $attempts -lt $max_attempts ]; do
        if apt update && apt full-upgrade -y && apt autoremove -y && apt autoclean; then
            log $LOG_LEVEL_INFO "System update and upgrade completed." "/var/log/khelp_proxy.log"
            return 0
        else
            log $LOG_LEVEL_ERROR "System update and upgrade failed. Retrying in $((attempts * 5)) seconds..." "/var/log/khelp_proxy.log"
            attempts=$((attempts + 1))
            sleep $((attempts * 5))
        fi
    done

    log $LOG_LEVEL_ERROR "System update and upgrade failed after $max_attempts attempts. Please check your network connection and try again." "/var/log/khelp_proxy.log"
    exit 1
}
...
if [ $? -ne 0 ]; then
    log $LOG_LEVEL_ERROR "Failed to enable hogen.service" "$HOGEN_LOG_FILE"
    exit 1
fi
