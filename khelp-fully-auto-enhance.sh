...
restore() {
    log "Restoring hostname and /etc/hosts from backup"
    if [ -f "$BACKUP_HOSTNAME_FILE" ]; then
        local original_hostname=$(cat "$BACKUP_HOSTNAME_FILE")
...
# Main function to change hostname
change_hostname() {
    local newhn=$(fetch_random_name)
    log "Setting new hostname to $newhn"

    # Change hostname
    if hostnamectl set-hostname "$newhn"; then
        log "Hostname changed successfully to $newhn"
    else
        log "Failed to change hostname"
        restore
        exit 1
    fi
