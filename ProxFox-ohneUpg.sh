...
log $LOG_LEVEL_INFO "Starting setup..." "$UPDATE_LOG_FILE"
...
install_tor() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        apt update -y && apt install -y tor
        if [ $? -eq 0 ]; then
...
install_sslh() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        apt update -y && apt install -y sslh
        if [ $? -eq 0 ]; then
...
install_proxychains() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        apt update -y && apt install -y proxychains
        if [ $? -eq 0 ]; then
...
install_logwatch() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        apt update -y && apt install -y logwatch
        if [ $? -eq 0 ]; then
...
install_rsyslog() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        apt update -y && apt install -y rsyslog
        if [ $? -eq 0 ]; then
...
fetch_proxies_with_fallback() {
    ...
    # First API Attempt
    while [ $attempts -lt $max_attempts ]; do
        log $LOG_LEVEL_INFO "Fetching new proxy list from $proxy_api_url1 (attempt $((attempts + 1)))..." "$PROXY_UPDATE_LOG_FILE"
        ...
        if [ -n "$response" ]; then
            ...
            if [ -n "$valid_proxies" ]; then
                ...
                log $LOG_LEVEL_INFO "Fetched $(echo "$valid_proxies" | wc -l) valid proxies." "$PROXY_UPDATE_LOG_FILE"
                return 0
            else
    ...
    # Second API Attempt
    log $LOG_LEVEL_INFO "Fallback: Fetching new proxy list from $proxy_api_url2..." "$PROXY_UPDATE_LOG_FILE"
