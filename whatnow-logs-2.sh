...
create_update_proxies_service() {
    ...
    # Ensure the proxychains.conf file exists and has default content if not
    if [ ! -f /etc/proxychains.conf ]; then
        log $LOG_LEVEL_INFO "Creating default proxychains.conf file..." "$UPDATE_LOG_FILE"
        mkdir -p /etc
        cat << EOF > /etc/proxychains.conf
# ProxyChains default configuration
# Dynamic chain
...
socks4  127.0.0.1 9050
EOF
        log $LOG_LEVEL_INFO "Created default proxychains.conf file." "$UPDATE_LOG_FILE"
    fi
