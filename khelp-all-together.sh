...
export hogen_log_file="/var/log/khelp_hogen.log"
...
log() {
    local level="$1"
    local message="$2"
    local hogen_log_file="/var/log/khelp_hogen.log"
...
# Documentation
mkdir -p /usr/local/share/khelp_hogen
cat << 'EOF' > /usr/local/share/khelp_hogen/README.md
...
## Log File
Logs are saved to /var/log/khelp_hogen.log. The log file is rotated if it exceeds 1MB.
...
log "INFO" ""
log "INFO" "khelp setups a TOR, UFW, Fail2Ban, SSHL, IPTABLES, PROXYCHAINS"
log "INFO" "It enables MSPOO - mac address spoof service."
log "INFO" "It enables HOGEN - Hostname spoof service."
log "Info" ""
