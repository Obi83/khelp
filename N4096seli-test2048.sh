...
install_packages() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        apt install -y curl tor ufw jq iptables fail2ban sslh proxychains openssl logwatch rsyslog nginx coreutils certbot python3-certbot-nginx selinux-utils selinux-basics policycoreutils
        if [ $? -eq 0 ]; then
...
configure_selinux() {
    log $LOG_LEVEL_INFO "Configuring SELinux for Tor..." "$UPDATE_LOG_FILE"

    # SELinux aktivieren
    if selinux-activate; then
        log $LOG_LEVEL_INFO "SELinux activated successfully." "$UPDATE_LOG_FILE"
    else
        log $LOG_LEVEL_ERROR "Failed to activate SELinux." "$UPDATE_LOG_FILE"
        return 1
    fi

    # SELinux in den Enforcing-Modus setzen
    if selinux-config-enforcing; then
        log $LOG_LEVEL_INFO "SELinux set to enforcing mode." "$UPDATE_LOG_FILE"
    else
        log $LOG_LEVEL_ERROR "Failed to set SELinux to enforcing mode." "$UPDATE_LOG_FILE"
        return 1
    fi

    # SELinux-Ports für Tor hinzufügen
    if semanage port -a -t tor_port_t -p tcp 9050; then
        log $LOG_LEVEL_INFO "Port 9050 configured in SELinux." "$UPDATE_LOG_FILE"
    else
        log $LOG_LEVEL_ERROR "Failed to configure port 9050 in SELinux. It might already be configured." "$UPDATE_LOG_FILE"
    fi

    if semanage port -a -t tor_port_t -p tcp 9051; then
        log $LOG_LEVEL_INFO "Port 9051 configured in SELinux." "$UPDATE_LOG_FILE"
    else
        log $LOG_LEVEL_ERROR "Failed to configure port 9051 in SELinux. It might already be configured." "$UPDATE_LOG_FILE"
    fi

    log $LOG_LEVEL_INFO "SELinux configured and enforcing mode enabled." "$UPDATE_LOG_FILE"
    return 0
}
...
# CookieAuthentication 0 - für hashpwd uncommend
# HashedControlPassword - für hashpwd uncommend
...
log $LOG_LEVEL_INFO "Validating Tor SELinux context..." "$UPDATE_LOG_FILE"
if ps -eZ | grep -q 'tor'; then
    log $LOG_LEVEL_INFO "Tor is running with SELinux context:" "$UPDATE_LOG_FILE"
    ps -eZ | grep 'tor' >> "$UPDATE_LOG_FILE"
else
    log $LOG_LEVEL_ERROR "Tor is not running or SELinux context is incorrect." "$UPDATE_LOG_FILE"
    exit 1
fi
