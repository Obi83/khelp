...
apt install -y kali-linux-large kali-tools-windows-resources terminator bpytop htop shellcheck seclists inxi ufw tor fastfetch guake impacket-scripts bloodhound powershell-empire
echo "khelp installed all useful helper tools."


# Environment variables for paths and configurations
export USER_HOME=${USER_HOME:-$(eval echo ~${SUDO_USER})}
export KALITORIFY_REPO=${KALITORIFY_REPO:-"https://github.com/brainfucksec/kalitorify.git"}
export STARTUP_SCRIPT_PATH=${STARTUP_SCRIPT_PATH:-"$USER_HOME/startup_script.sh"}
export DESKTOP_ENTRY_PATH=${DESKTOP_ENTRY_PATH:-"$USER_HOME/.config/autostart/startup_terminal.desktop"}


# Download and install kalitorify
echo "Downloading and installing kalitorify..."
git clone "$KALITORIFY_REPO" "$USER_HOME/kalitorify"
...
# Create and set permissions for kalitorify scripts
echo "Creating kalitorify startup and restart scripts..."
...
# Create and enable kalitorify systemd service
echo "Creating and enabling kalitorify systemd service..."
