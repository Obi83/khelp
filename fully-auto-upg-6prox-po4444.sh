...
# Update SSHD configuration to listen on port 4444, set PasswordAuthentication to yes, set MaxAuthTries to 3, set MaxSessions to 7, and set PermitRootLogin to yes
echo "Updating SSHD configuration to listen on port 4444, set PasswordAuthentication to yes, set MaxAuthTries to 3, set MaxSessions to 7, and set PermitRootLogin to yes..."
...
sed -i 's/^MaxAuthTries [0-9]*/MaxAuthTries 3/' /etc/ssh/sshd_config
sed -i 's/^#MaxSessions [0-9]*/MaxSessions 7/' /etc/ssh/sshd_config
sed -i 's/^MaxSessions [0-9]*/MaxSessions 7/' /etc/ssh/sshd_config
...
[Desktop Entry]
Type=Application
Exec=terminator -e "bash -c 'sudo $HOME/startup_script.sh; exec bash'"
Hidden=false
NoDisplay=false
