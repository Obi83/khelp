...
update_system() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            if sudo dnf upgrade --refresh -y && sudo dnf autoremove -y; then
                log $LOG_LEVEL_INFO "System update and upgrade completed." "$UPDATE_LOG_FILE"
                return 0
            fi
        elif [ -x "$(command -v zypper)" ]; then
            if sudo zypper refresh && sudo zypper update -y && sudo zypper clean && sudo zypper remove -u -y; then
                log $LOG_LEVEL_INFO "System update and upgrade completed." "$UPDATE_LOG_FILE"
                return 0
            fi
        elif [ -x "$(command -v pacman)" ]; then
            if sudo pacman -Syu --noconfirm && sudo pacman -Rns $(pacman -Qdtq) --noconfirm && sudo pacman -Sc --noconfirm; then
                log $LOG_LEVEL_INFO "System update and upgrade completed." "$UPDATE_LOG_FILE"
                return 0
            fi
        else
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please update the system manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_curl() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install curl manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_tor() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install tor manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_ufw() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install ufw manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_jq() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install jq manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_iptables() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install iptables manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_fail2ban() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install fail2ban manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_sslh() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            sudo zypper install -y sslh
        elif [ -x "$(command -v pacman)" ]; then
            sudo pacman -Syu --noconfirm sslh
        else
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install sslh manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_proxychains() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            sudo zypper install -y proxychains
        elif [ -x "$(command -v pacman)" ]; then
            sudo pacman -Syu --noconfirm proxychains
        else
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install proxychains manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_openssl() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install openssl manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_logwatch() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            sudo zypper install -y logwatch
        elif [ -x "$(command -v pacman)" ]; then
            sudo pacman -Syu --noconfirm logwatch
        else
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install logwatch manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_rsyslog() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install rsyslog manually." "$UPDATE_LOG_FILE"
            return 1
        fi
...
install_git() {
    log $LOG_LEVEL_INFO "Installing git..." "$UPDATE_LOG_FILE"
    local attempts=0
    local max_attempts=3
    ...
    while [ $attempts -lt $max_attempts ]; do
        if [ -x "$(command -v apt)" ]; then
            ...
            log $LOG_LEVEL_ERROR "Unsupported package manager. Please install git manually." "$UPDATE_LOG_FILE"
            return 1
        fi

        if [ $? -eq 0 ]; then
            log $LOG_LEVEL_INFO "Git installed successfully." "$UPDATE_LOG_FILE"
            return 0
        else
            attempts=$((attempts + 1))
            log $LOG_LEVEL_ERROR "Failed to install git. Attempt $attempts of $max_attempts. Retrying in $((attempts * 5)) seconds..." "$UPDATE_LOG_FILE"
            sleep $((attempts * 5))
        fi
    done

    log $LOG_LEVEL_ERROR "Failed to install git after $max_attempts attempts. Please check your network connection and try again." "$UPDATE_LOG_FILE"
    return 1
}
...
configure_fail2ban() {
    ...
    log $LOG_LEVEL_INFO "Creating/Updating jail.local configuration..." "$UPDATE_LOG_FILE"
