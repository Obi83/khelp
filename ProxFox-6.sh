...
fetch_proxies() {
    mkdir -p "$(dirname "$PROXY_LIST_FILE")" || { log $LOG_LEVEL_ERROR "Failed to create directory for proxy list file." "$UPDATE_LOG_FILE"; exit 1; }

    original_ip=$(curl -s https://api.ipify.org) || { log $LOG_LEVEL_ERROR "Failed to fetch original IP." "$UPDATE_LOG_FILE"; exit 1; }
    
    fetch_and_validate_proxies() {
        local api_url=$1
        log $LOG_LEVEL_INFO "Fetching new proxy list from $api_url..." "$UPDATE_LOG_FILE"
        ...
        if [ -n "$response" ]; then
            ...
            if [ -n "$valid_proxies" ]; then
                ...
                log $LOG_LEVEL_INFO "Fetched and validated $(grep -c '' "$PROXY_LIST_FILE") valid proxies from $api_url." "$UPDATE_LOG_FILE"
            else
                log $LOG_LEVEL_ERROR "No valid proxies found in the response from $api_url." "$UPDATE_LOG_FILE"
            fi
        else
            log $LOG_LEVEL_ERROR "Failed to fetch proxies from $api_url or the response is empty." "$UPDATE_LOG_FILE"
        fi
    }

    # Abrufen und Validieren von Proxies von allen APIs
