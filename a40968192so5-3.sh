...
echo "| | _| |__   ___| |_ __         _| || |_       |  _ \ _ __ _____  __/ _| _____  __ "
...
# Funktion definieren
update_certificates() {
    log $LOG_LEVEL_INFO "Updating CA certificates..." "$UPDATE_LOG_FILE"

    # Überprüfe, ob das Zertifikat existiert
    if [ -f "nginx-selfsigned.crt" ]; then
        chmod 600 nginx-selfsigned.crt
        cp nginx-selfsigned.crt /usr/local/share/ca-certificates/

        # Aktualisiere die Zertifikate
        update-ca-certificates
        if [ $? -eq 0 ]; then
            log $LOG_LEVEL_INFO "CA certificates updated successfully." "$UPDATE_LOG_FILE"
        else
            log $LOG_LEVEL_ERROR "Failed to update CA certificates." "$UPDATE_LOG_FILE"
        fi
    else
        log $LOG_LEVEL_ERROR "nginx-selfsigned.crt not found. Skipping certificate update." "$UPDATE_LOG_FILE"
    fi
}
...
configure_iptables() {
    ...
    for port in "${important_ports[@]}"; do
        if [[ "$port" -eq 9050 ]]; then
            # Allow outgoing traffic only on port 9050 (SOCKS5)
            iptables -A OUTPUT -p tcp --dport "$port" -j ACCEPT
            iptables -A INPUT -p tcp --sport "$port" -j ACCEPT
            log $LOG_LEVEL_INFO "Allowed traffic on port $port (IPv4)." "$IPTABLES_LOG_FILE"
        else
            # Explicitly block all other ports
            iptables -A INPUT -p tcp --dport "$port" -j DROP
            iptables -A OUTPUT -p tcp --dport "$port" -j DROP
            log $LOG_LEVEL_INFO "Blocked traffic on port $port (IPv4)." "$IPTABLES_LOG_FILE"
        fi
    done
    ...
    for port in "${important_ports[@]}"; do
        if [[ "$port" -eq 9050 ]]; then
            ...
            log $LOG_LEVEL_INFO "Allowed traffic on port $port (IPv6)." "$IPTABLES_LOG_FILE"
        else
            # Explicitly block all other ports
...
services=("update_proxies" "iptables" "ufw" "tor" "fail2ban" "rsyslog" "update_proxies.timer")
...
# Funktion zur Extraktion von IP-Leak-Informationen
function ipleak_info() {
  local output
  local raw_data
  local response_code

  # Abrufen von Daten von ipleak.net
  raw_data=$(proxychains curl -s -w "%{http_code}" -o /tmp/ipleak_response.html https://ipleak.net)
  response_code=${raw_data: -3}  # Extrahiere die letzten 3 Zeichen als HTTP-Statuscode
  raw_data=$(cat /tmp/ipleak_response.html)

  # Debugging: Speichere die Rohdaten in einer Datei
  echo "$raw_data" > /var/log/ipleak_raw_response.log

  # Prüfe HTTP-Statuscode
  if [[ "$response_code" -ne 200 ]]; then
    log $LOG_LEVEL_ERROR "Fehler beim Abrufen von ipleak.net. HTTP-Status: $response_code. Rohdaten gespeichert in /var/log/ipleak_raw_response.log" "$UPDATE_LOG_FILE"
    return 1
  fi

  # Extrahiere JSON-Daten aus der Rohantwort
  output=$(grep 'id="jsonData"' /tmp/ipleak_response.html | \
  ...
  if [[ -z "$output" ]]; then
    log $LOG_LEVEL_ERROR "Fehler bei der Extraktion der IP-Leak-Informationen. Rohdaten gespeichert in /var/log/ipleak_raw_response.log" "$UPDATE_LOG_FILE"
    return 1
  fi
