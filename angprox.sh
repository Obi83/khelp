...
install_certbot() {
    log $LOG_LEVEL_INFO "Installing certbot..." "$UPDATE_LOG_FILE"
    local attempts=0
    local max_attempts=3
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if [ $? -eq 0 ]; then
            log $LOG_LEVEL_INFO "certbot installed successfully." "$UPDATE_LOG_FILE"
            return 0
        else
            attempts=$((attempts + 1))
            log $LOG_LEVEL_ERROR "Failed to install certbot. Attempt $attempts of $max_attempts. Retrying in $((attempts * 5)) seconds..." "$UPDATE_LOG_FILE"
            sleep $((attempts * 5))
        fi
    done

    log $LOG_LEVEL_ERROR "Failed to install certbot after $max_attempts attempts. Please check your network connection and try again." "$UPDATE_LOG_FILE"
    return 1
}
...
configure_iptables() {
    ...
    log $LOG_LEVEL_INFO "Set default policies for INPUT, FORWARD, and OUTPUT chains." "$IPTABLES_LOG_FILE"

    # Allow loopback traffic and established connections
    iptables -A INPUT -i lo -j ACCEPT
    iptables -A INPUT -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
    log $LOG_LEVEL_INFO "Configured loopback and established connections." "$IPTABLES_LOG_FILE"
    ...
    log $LOG_LEVEL_INFO "Allowed SSH, Tor, and HTTPS ports." "$IPTABLES_LOG_FILE"
    ...
    log $LOG_LEVEL_INFO "Dropped invalid packets." "$IPTABLES_LOG_FILE"
...
fetch_proxies() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        for proxy_api_url in "${proxy_api_urls[@]}"; do
            ...
            if [ -n "$response" ]; then
                local valid_proxies=$(printf "%s" "$response" | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+:[0-9]+' | head -n $max_proxies)
                if [ -z "$valid_proxies" ]; then
                    log $LOG_LEVEL_ERROR "No valid proxies to process." "$UPDATE_LOG_FILE"
                    continue
                fi
                
                printf "%s\n" "$valid_proxies" | while IFS= read -r proxy; do
        ...
        if [ ${#all_proxies[@]} -gt 0 ]; then
            if ! printf "%s\n" "${all_proxies[@]}" > "$proxy_list_file"; then
                log $LOG_LEVEL_ERROR "Failed to write proxies to $proxy_list_file" "$UPDATE_LOG_FILE"
                return 1
            fi
            log $LOG_LEVEL_INFO "Fetched ${#all_proxies[@]} valid proxies." "$UPDATE_LOG_FILE"
            return 0
        fi
