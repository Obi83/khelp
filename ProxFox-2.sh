...
mkdir -p /var/log/proxfox
mkdir -p /usr/local/share/proxfox

# Environment variables for paths and configurations
#Log Files
export UPDATE_LOG_FILE="/var/log/proxfox/proxfox.log"
export PROXY_UPDATE_LOG_FILE="/var/log/proxfox/update_proxies.log"
export PROXY_TIMER_LOG_FILE="/var/log/proxfox/timer_proxies.log"
export IPTABLES_LOG_FILE="/var/log/proxfox/iptables.log"

# Directories
export PROXFOX_UPDATE_DIR="/usr/local/share/proxfox/update"
export PROXFOX_INSTALLER_DIR="/usr/local/share/proxfox/installer"
export PROXFOX_PROXYCHAINS_DIR="/usr/local/share/proxfox/proxychains"
export PROXFOX_UFW_DIR="/usr/local/share/proxfox/ufw"
export PROXFOX_FAIL2BAN_DIR="/usr/local/share/proxfox/fail2ban"
export PROXFOX_IPTABLES_DIR="/usr/local/share/proxfox/iptables"
export PROXFOX_TOR_DIR="/usr/local/share/proxfox/tor"
export PROXFOX_LOGGING_DIR="/usr/local/share/proxfox/logging"
...
LOG_LEVEL_INFO=0
LOG_LEVEL_ERROR=1
UPDATE_LOG_FILE="/var/log/proxfox/update_proxies.log"
...
fetch_proxies_with_fallback() {
    ...
    # First API Attempt
    while [ $attempts -lt $max_attempts ]; do
        log $LOG_LEVEL_INFO "Fetching new proxy list from $proxy_api_url1 (attempt $((attempts + 1)))..." "$UPDATE_LOG_FILE"
        ...
        if [ -n "$response" ]; then
            ...
            if [ -n "$valid_proxies" ]; then
                ...
                log $LOG_LEVEL_ERROR "No valid proxies found in the response from $proxy_api_url1." "$UPDATE_LOG_FILE"
            fi
        else
            log $LOG_LEVEL_ERROR "Failed to fetch proxies from $proxy_api_url1 or the response is empty." "$UPDATE_LOG_FILE"
        fi
        attempts=$((attempts + 1))
        sleep 5
    done
    ...
    # Second API Attempt
    log $LOG_LEVEL_INFO "Fallback: Fetching new proxy list from $proxy_api_url2..." "$UPDATE_LOG_FILE"
    local response=$(curl -s $proxy_api_url2)
    if [ -n "$response" ]; then
        local valid_proxies=$(echo "$response" | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+:[0-9]+' | head -n $max_proxies)
        if [ -n "$valid_proxies" ]; then
            echo "$valid_proxies" > "$proxy_list_file"
            log $LOG_LEVEL_INFO "Fetched $(echo "$valid_proxies" | wc -l) valid proxies from the fallback API." "$UPDATE_LOG_FILE"
            return 0
        else
            log $LOG_LEVEL_ERROR "No valid proxies found in the response from $proxy_api_url2." "$UPDATE_LOG_FILE"
        fi
    else
        log $LOG_LEVEL_ERROR "Failed to fetch proxies from $proxy_api_url2 or the response is empty." "$UPDATE_LOG_FILE"
    fi

    # Third API Attempt
    log $LOG_LEVEL_INFO "Fallback: Fetching new proxy list from $proxy_api_url3..." "$UPDATE_LOG_FILE"
    ...
    if [ -n "$response" ]; then
        ...
        if [ -n "$valid_proxies" ]; then
            ...
            log $LOG_LEVEL_ERROR "No valid proxies found in the response from $proxy_api_url3." "$UPDATE_LOG_FILE"
        fi
    else
        log $LOG_LEVEL_ERROR "Failed to fetch proxies from $proxy_api_url3 or the response is empty." "$UPDATE_LOG_FILE"
    fi

    log $LOG_LEVEL_ERROR "Failed to fetch proxies after attempting all APIs. Exiting." "$UPDATE_LOG_FILE"
    exit 1
}
