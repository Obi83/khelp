...
update_system() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if apt update && apt full-upgrade -y && apt autoremove -y && apt autoclean; then
            ...
            log "ERROR" "System update and upgrade failed. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi
    done

    log "ERROR" "System update and upgrade failed after $max_attempts attempts."
    exit 1
}
...
install_packages() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if sudo apt install -y $packages; then
            ...
            log "ERROR" "Package installation failed. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi
    done

    log "ERROR" "Package installation failed after $max_attempts attempts."
    exit 1
}
...
if ! command -v proxychains &> /dev/null; then
    ...
    while [ $attempts -lt $max_attempts ]; do
        if apt install -y proxychains; then
            ...
            log "ERROR" "Failed to install ProxyChains. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi

        if [ $attempts -eq $max_attempts ]; then
            log "ERROR" "Failed to install ProxyChains after $max_attempts attempts."
            exit 1
        fi
    done
else
...
configure_fail2ban() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if apt install -y fail2ban; then
            ...
            log "ERROR" "Failed to install Fail2ban. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi

        if [ $attempts -eq $max_attempts ]; then
            log "ERROR" "Failed to install Fail2ban after $max_attempts attempts."
            exit 1
        fi
    done
...
configure_tor() {
    ...
    while [ $attempts -lt $max_attempts ]; do
        if apt install -y tor; then
            ...
            log "ERROR" "Failed to install Tor. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi

        if [ $attempts -eq $max_attempts ]; then
            log "ERROR" "Failed to install Tor after $max_attempts attempts."
            exit 1
        fi
    done
...
if ! command -v curl &> /dev/null; then
    ...
    while [ $attempts -lt $max_attempts ]; do
        if apt install -y curl; then
            ...
            log "ERROR" "Failed to install curl. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi

        if [ $attempts -eq $max_attempts ]; then
            log "ERROR" "Failed to install curl after $max_attempts attempts."
            exit 1
        fi
    done
else
...
if ! command -v jq &> /dev/null; then
    ...
    while [ $attempts -lt $max_attempts ]; do
        if apt install -y jq; then
            ...
            log "ERROR" "Failed to install jq. Retrying..."
            attempts=$((attempts + 1))
            sleep 5
        fi

        if [ $attempts -eq $max_attempts ]; then
            log "ERROR" "Failed to install jq after $max_attempts attempts."
            exit 1
        fi
    done
else
...
# Log the completion of hostname generator service creation and enabling
log "INFO" "Hostname generator service created and enabled."
