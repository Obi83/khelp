...
fetch_proxies() {
  ...
  local temp_proxy_list_file="${proxy_list_file}.tmp"
  local max_proxies=100
  local attempts=0
  local max_attempts=3

  mkdir -p "$(dirname "$proxy_list_file")"
  : > "$temp_proxy_list_file"  # Temporäre Datei leeren oder erstellen

  original_ip=$(curl -s https://api.ipify.org)
  
  # Funktion zum Abrufen und Validieren von Proxies
        ...
        if [ -n "$valid_proxies" ]; then
            while IFS= read -r proxy; do
                ...
                if [ "$original_ip" != "$proxy_ip" ]; then
                    echo "$proxy" >> "$temp_proxy_list_file"
                fi
            done <<< "$valid_proxies"
            log $LOG_LEVEL_INFO "Fetched and validated $(cat "$temp_proxy_list_file" | wc -l) valid proxies from $api_url." "$PROXY_UPDATE_LOG_FILE"
        else
  ...
  # Doppelte Proxies entfernen und in die endgültige Datei schreiben
  sort -u "$temp_proxy_list_file" > "$proxy_list_file"
  rm "$temp_proxy_list_file"
}
